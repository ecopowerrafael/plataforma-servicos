...
export class EvolutionWhatsAppClient {
  ...
  private async request<T>(path: string, init: RequestInit = {}, requestApiKey?: string): Promise<T> {
    ...
    if (!response.ok) { const details = Array.isArray(body) ? body.filter((item): item is { path: string; message: string } => Boolean(item && typeof item === 'object' && 'path' in item && 'message' in item)) : []; const bodyObject = body !== null && typeof body === 'object' && !Array.isArray(body) ? body as Record<string, unknown> : null; console.error('[EVOLUTION_HTTP]', JSON.stringify({ path, upstreamStatus: response.status, bodyKeys: bodyObject === null ? [] : Object.keys(bodyObject).slice(0, 12), detailCount: details.length, errorCode: typeof bodyObject?.code === 'string' ? bodyObject.code : null, errorMessagePresent: typeof bodyObject?.message === 'string' || typeof bodyObject?.error === 'string' })); throw new AppError({ code: response.status === 401 || response.status === 403 ? 'EVOLUTION_UNAUTHORIZED' : 'EVOLUTION_PROVIDER_ERROR', message: `A Evolution respondeu HTTP ${response.status}.`, statusCode: response.status === 401 || response.status === 403 ? 401 : 502, ...(details.length > 0 ? { details } : {}) }); }
