...
export class EvolutionWhatsAppClient {
  ...
  public sendList(instanceToken: string, number: string, text: string, rows: Array<{ rowId: string; title: string }>) { return this.request<{ id?: string; messageId?: string }>('/send/list', { method: 'POST', body: JSON.stringify({ number, description: text, buttonText: 'Ver opções', sections: [{ title: 'Opções', rows }] }) }, instanceToken); }
}
