...
export class EvolutionWhatsAppProvisioning implements WhatsAppProvisioningProvider {
  ...
  private webhookUrl(webhookPublicId: string | null) { if (!webhookPublicId) throw new AppError({ code: 'EVOLUTION_WEBHOOK_UNAVAILABLE', message: 'Webhook público da Evolution não configurado.', statusCode: 503 }); const base = process.env.APP_WEB_URL?.trim() || 'http://localhost:3000'; return `${base.replace(/\/$/u, '')}/webhooks/whatsapp/evolution/${webhookPublicId}`; }
  ...
  public async qr(tenantId: bigint) { const row = await this.config(tenantId); if (!row) throw new AppError({ code: 'WHATSAPP_NOT_CONFIGURED', message: 'Crie a conexão antes de solicitar o QR Code.', statusCode: 400 }); const token = this.token(row); await this.evolution.connect(token, this.webhookUrl(row.webhookPublicId)); const result = await this.evolution.qr(token); const qrCode = result.qrCode ?? result.code ?? result.Qrcode ?? result.Code; if (!qrCode) throw new AppError({ code: 'EVOLUTION_PROVIDER_ERROR', message: 'A Evolution não retornou o QR Code.', statusCode: 502 }); return { qrCode: qrCode.startsWith('data:') ? qrCode : `data:image/png;base64,${qrCode}`, view: this.view(await this.client.tenantWhatsAppConfig.update({ where: { tenantId_provider: { tenantId, provider: 'EVOLUTION' } }, data: { connectionStatus: 'WAITING_QR' } })) }; }
  ...
  public async reconnect(tenantId: bigint) { const row = await this.config(tenantId); if (!row) throw new AppError({ code: 'WHATSAPP_NOT_CONFIGURED', message: 'Conexão Evolution não configurada.', statusCode: 400 }); const token = this.token(row); await this.evolution.connect(token, this.webhookUrl(row.webhookPublicId)); const result = await this.evolution.reconnect(token); const qrCode = result.qrCode ?? result.code ?? result.Qrcode ?? result.Code ?? ''; if (!qrCode) throw new AppError({ code: 'EVOLUTION_QR_UNAVAILABLE', message: 'A Evolution não disponibilizou um novo QR Code.', statusCode: 502 }); const saved = await this.client.tenantWhatsAppConfig.update({ where: { tenantId_provider: { tenantId, provider: 'EVOLUTION' } }, data: { connectionStatus: 'WAITING_QR', active: false } }); return { qrCode: qrCode.startsWith('data:') ? qrCode : `data:image/png;base64,${qrCode}`, view: this.view(saved) }; }
}
