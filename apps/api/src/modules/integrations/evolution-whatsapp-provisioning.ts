...
export class EvolutionWhatsAppProvisioning implements WhatsAppProvisioningProvider {
  ...
  public async refreshStatus(tenantId: bigint) {
    const row = await this.config(tenantId);
    if (!row) return this.view(null);
    const now = new Date();
    try {
      const result = await this.evolution.status(this.token(row));
      ...
      const connectedPhone = result.connectedPhone ?? result.phone ?? result.number ?? (jidPhone || undefined);
      const connectedName = result.connectedName ?? result.name ?? result.Name ?? result.pushName;
      return this.view(await this.client.tenantWhatsAppConfig.update({ where: { tenantId_provider: { tenantId, provider: 'EVOLUTION' } }, data: { active: connected, connectionStatus: state, lastStatusCheckAt: now, ...(connected && row.connectedAt === null ? { connectedAt: now } : {}), ...(connectedPhone !== undefined ? { connectedPhone } : {}), ...(connectedName !== undefined ? { connectedName } : {}) } }));
    } catch {
      const updated = await this.client.tenantWhatsAppConfig.update({ where: { tenantId_provider: { tenantId, provider: 'EVOLUTION' } }, data: { active: false, connectionStatus: 'ERROR', lastStatusCheckAt: now } });
  ...
  public async reconnect(tenantId: bigint) { const row = await this.config(tenantId); if (!row) throw new AppError({ code: 'WHATSAPP_NOT_CONFIGURED', message: 'Conexão Evolution não configurada.', statusCode: 400 }); await this.refreshStatus(tenantId); const current = await this.config(tenantId); if (!current) throw new AppError({ code: 'WHATSAPP_NOT_CONFIGURED', message: 'Conexão Evolution não configurada.', statusCode: 400 }); const token = this.token(current); await this.evolution.reconnect(token); await this.ensureWebhookAndSettings(current); const result = await this.evolution.qr(token); const qrCode = result.qrCode ?? result.qrcode ?? result.code ?? result.Qrcode ?? result.Code ?? ''; if (!qrCode) throw new AppError({ code: 'EVOLUTION_QR_UNAVAILABLE', message: 'A Evolution não disponibilizou um novo QR Code.', statusCode: 502 }); const saved = await this.client.tenantWhatsAppConfig.update({ where: { tenantId_provider: { tenantId, provider: 'EVOLUTION' } }, data: { connectionStatus: 'WAITING_QR', active: false } }); return { qrCode: qrCode.startsWith('data:') ? qrCode : `data:image/png;base64,${qrCode}`, view: this.view(saved) }; }
