...
export const directoryRoutes: FastifyPluginAsyncZod<DirectoryRoutesOptions> = async (app, options) => {
  ...
  app.get('/platform/directory/metrics', { schema: { querystring: z.object({ from: z.coerce.date().optional(), to: z.coerce.date().optional(), categorySlug: z.string().optional(), state: z.string().length(2).optional(), city: z.string().optional(), search: z.string().optional(), hasTenant: z.enum(['true', 'false']).transform((value) => value === 'true').optional() }) } }, (request) => { allow(request, 'platform.tenant.read'); return options.service.metrics(request.query); });
  app.get('/platform/directory/metrics.csv', { schema: { querystring: z.object({ from: z.coerce.date().optional(), to: z.coerce.date().optional() }) } }, async (request, reply) => { allow(request, 'platform.tenant.read'); const metrics = await options.service.metrics(request.query); const quote = (value: string | number | boolean | Date | null) => `"${String(value ?? '').replace(/"/gu, '""')}"`; const lines = [['empresa','categoria','cidade','telefone','whatsapp','cliques','cliques_unicos','visualizacoes','ctr','ultimo_clique','tenant_vinculado'].join(','), ...metrics.rows.map((row) => [row.business,row.category,`${row.city}/${row.state}`,row.phone,row.whatsapp,row.whatsappClicks,row.uniqueWhatsappClicks,row.pageViews,row.whatsappCtr,row.lastWhatsappClickAt,row.tenantLinked].map(quote).join(','))]; return reply.type('text/csv; charset=utf-8').send(lines.join('\n')); });
};
