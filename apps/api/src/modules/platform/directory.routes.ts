...
function overviewInput(query: z.infer<typeof seoQuery>) { return { ...(query.from === undefined ? {} : { from: query.from }), ...(query.to === undefined ? {} : { to: query.to }), ...(query.categorySlug === undefined ? {} : { categorySlug: query.categorySlug }), ...(query.city === undefined ? {} : { city: query.city }), ...(query.search === undefined ? {} : { search: query.search }), ...(query.hasTenant === undefined ? {} : { hasTenant: query.hasTenant }) }; }
function queriesInput(query: z.infer<typeof seoQuery>) { return { ...(query.from === undefined ? {} : { from: query.from }), ...(query.to === undefined ? {} : { to: query.to }), ...(query.categorySlug === undefined ? {} : { categorySlug: query.categorySlug }), ...(query.citySlug === undefined ? {} : { citySlug: query.citySlug }) }; }
...
export const directoryRoutes: FastifyPluginAsyncZod<DirectoryRoutesOptions> = async (app, options) => {
  ...
  app.get('/platform/directory/seo/overview', { schema: { querystring: seoQuery } }, (request) => { allow(request, 'platform.tenant.read'); return seo().overview(overviewInput(request.query)); });
  app.get('/platform/directory/seo/businesses', { schema: { querystring: seoQuery } }, async (request) => { allow(request, 'platform.tenant.read'); return (await seo().overview(overviewInput(request.query))).rows; });
  app.get('/platform/directory/seo/queries', { schema: { querystring: seoQuery } }, (request) => { allow(request, 'platform.tenant.read'); return seo().queries(queriesInput(request.query)); });
  ...
  app.get('/platform/directory/seo/businesses.csv', { schema: { querystring: seoQuery } }, async (request, reply) => { allow(request, 'platform.tenant.read'); const rows = (await seo().overview(overviewInput(request.query))).rows; const quote = (value: string | number | boolean) => `"${String(value).replace(/"/gu, '""')}"`; const lines = [['empresa','categoria','cidade','uf','impressoes','google_clicks','google_ctr','position','page_views','whatsapp_clicks','unique_whatsapp_clicks','whatsapp_conversion','tenant_linked'].join(','), ...rows.map((row) => [row.business,row.category,row.city,row.state,row.impressions,row.googleClicks,row.googleCtr,row.position,row.pageViews,row.whatsappClicks,row.uniqueWhatsappClicks,row.whatsappConversion,row.tenantLinked].map(quote).join(','))]; return reply.type('text/csv; charset=utf-8').send(lines.join('\n')); });
