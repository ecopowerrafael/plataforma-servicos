...
export class DirectoryService {
  public constructor(private readonly client: PrismaClient, private readonly seo?: DirectorySeoService) {}
  ...
  public async recordEvent(publicId: string, input: { type: 'BUSINESS_VIEW' | 'WHATSAPP_CLICK'; visitorId?: string | undefined; sessionId?: string | undefined; sourcePath: string; referrer?: string | undefined; utmSource?: string | undefined; utmMedium?: string | undefined; utmCampaign?: string | undefined }) {
    const business = await this.client.directoryBusiness.findFirst({ where: { publicId, active: true }, select: { id: true } });
  ...
  public async metrics(input: { from?: Date | undefined; to?: Date | undefined; categorySlug?: string | undefined; state?: string | undefined; city?: string | undefined; search?: string | undefined; hasTenant?: boolean | undefined; businessPublicId?: string | undefined } = {}) {
    const endOfDay = input.to === undefined ? undefined : new Date(Date.UTC(input.to.getUTCFullYear(), input.to.getUTCMonth(), input.to.getUTCDate(), 23, 59, 59, 999));
  ...
  public async importStatus(publicId: string) {
    ...
    const categoryStatusCounts = await this.client.directoryImportItem.groupBy({ by: ['categoryDetected', 'status'], where: { importId: directoryImport.id }, _count: true });
    const count = (status: string) => counts.find((item) => item.status === status)?._count ?? 0;
    const processedCount = count('CREATED') + count('UPDATED') + count('UNCHANGED') + count('POSSIBLE_DUPLICATE') + count('ERROR');
    const remaining = Math.max(directoryImport.totalSelected - processedCount, 0);
    const categoryCount = (detected: string, status: string) => categoryStatusCounts.find((item) => (item.categoryDetected ?? 'Não classificado') === detected && item.status === status)?._count ?? 0;
    return { import: { publicId: directoryImport.publicId, filename: directoryImport.filename, status: remaining === 0 && directoryImport.totalSelected > 0 ? 'COMPLETED' : directoryImport.status, totalFound: directoryImport.totalFound, totalSelected: directoryImport.totalSelected, processedCount, totalCreated: count('CREATED'), totalUpdated: count('UPDATED'), totalUnchanged: count('UNCHANGED'), totalDuplicates: count('POSSIBLE_DUPLICATE'), errors: count('ERROR'), remaining, progressPercent: directoryImport.totalSelected === 0 ? 0 : Math.round(processedCount / directoryImport.totalSelected * 10_000) / 100 }, categories: grouped.map((item) => { const detected = item.categoryDetected ?? 'Não classificado'; const category = categoryByName.get(detected); return { slug: category?.slug ?? null, name: category?.pluralName ?? detected, detected, count: item._count, existing: category !== undefined, created: categoryCount(detected, 'CREATED'), updated: categoryCount(detected, 'UPDATED'), unchanged: categoryCount(detected, 'UNCHANGED'), duplicates: categoryCount(detected, 'POSSIBLE_DUPLICATE') }; }) };
  }
  ...
  public async configure(publicId: string, assignments: { detected: string; categorySlug: string }[], newCategories: { name: string; singularName: string; pluralName: string; slug: string; active?: boolean | undefined; indexable?: boolean | undefined }[]) {
    const directoryImport = await this.client.directoryImport.findUnique({ where: { publicId } });
    if (directoryImport === null) throw new AppError({ code: 'DIRECTORY_IMPORT_NOT_FOUND', message: 'Importação não encontrada.', statusCode: 404 });
    if (directoryImport.processedCount > 0 || ['PROCESSING', 'PAUSED', 'COMPLETED'].includes(directoryImport.status)) throw new AppError({ code: 'DIRECTORY_IMPORT_ALREADY_STARTED', message: 'A importação já foi iniciada e não pode ser reconfigurada.', statusCode: 409 });
  ...
  public async processBatch(publicId: string, batchSize = Math.min(Math.max(Number(process.env.DIRECTORY_IMPORT_BATCH_SIZE ?? 25), 1), 100)) {
    ...
    const processedCount = count('CREATED') + count('UPDATED') + count('UNCHANGED') + count('POSSIBLE_DUPLICATE') + count('ERROR');
    await this.client.directoryImport.update({ where: { id: directoryImport.id }, data: { processedCount, totalCreated: count('CREATED'), totalUpdated: count('UPDATED'), totalUnchanged: count('UNCHANGED'), totalDuplicates: count('POSSIBLE_DUPLICATE'), status: remaining === 0 ? 'COMPLETED' : 'QUEUED', ...(remaining === 0 ? { completedAt: new Date() } : {}) } });
  ...
  public async retryErrors(publicId: string) { const directoryImport = await this.client.directoryImport.findUnique({ where: { publicId } }); if (directoryImport === null) throw new AppError({ code: 'DIRECTORY_IMPORT_NOT_FOUND', message: 'Importação não encontrada.', statusCode: 404 }); const retried = await this.client.directoryImportItem.updateMany({ where: { importId: directoryImport.id, status: 'ERROR' }, data: { status: 'SKIPPED', message: null } }); await this.client.directoryImport.update({ where: { id: directoryImport.id }, data: { status: 'QUEUED', completedAt: null, ...(retried.count > 0 ? { processedCount: { decrement: retried.count } } : {}) } }); return this.preview(publicId); }
