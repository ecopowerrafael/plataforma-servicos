...
export class PlatformBillingService {
  ...
  public async overview(){const configs=await this.client.platformPaymentConfig.findMany();const items: Array<{provider:'pix-local'|'mercadopago'|'stripe';active:boolean;environment:PaymentGatewayEnvironment;hasCredentials:boolean;keyType:string|null;receiverName:string|null;city:string|null;updatedAt:string}>=providers.map(provider=>{const config=configs.find(c=>c.provider===provider);const visible=config?.credentialsCiphertext&&this.cipher?this.cipher.decrypt(config.credentialsCiphertext):{};return {provider,active:config?.active??false,environment:config?.environment??'SANDBOX',hasCredentials:config?.credentialsCiphertext!==null&&config!==undefined,keyType:typeof visible.keyType==='string'?visible.keyType:null,receiverName:typeof visible.receiverName==='string'?visible.receiverName:null,city:typeof visible.city==='string'?visible.city:null,updatedAt:(config?.updatedAt??new Date(0)).toISOString()};});const stripeConfigured=Boolean(process.env.STRIPE_SECRET_KEY&&process.env.STRIPE_WEBHOOK_SECRET);items.push({provider:'stripe',active:stripeConfigured,environment:process.env.STRIPE_SECRET_KEY?.startsWith('sk_live_')?'PRODUCTION':'SANDBOX',hasCredentials:stripeConfigured,keyType:null,receiverName:null,city:null,updatedAt:new Date(0).toISOString()});return PlatformFinanceOverviewSchema.parse({configs:items,manualActivationEnabled:configs.find(c=>c.provider==='manual')?.active??true});}
