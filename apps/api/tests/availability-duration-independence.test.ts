...
describe.skipIf(url === undefined)('duração de serviço é independente do intervalo 45', () => {
  ...
  beforeEach(async () => {
    const user = await client.user.create({
      data: {
        publicId: randomUUID(),
        email: `test-dur-${randomUUID()}@test.invalid`,
        normalizedEmail: `test-dur-${randomUUID()}@test.invalid`,
        passwordHash: 'test',
        status: 'ACTIVE',
      },
    });
    ...
    const tenant = await client.tenant.create({
      data: {
        publicId: randomUUID(),
        slug: `avail-dur-${suffix}-${randomUUID().slice(0, 4)}`,
  ...
  it('permite 90 minutos de duração com 45 minutos de intervalo de granularidade', async () => {
    ...
    // Verificar que slots começam a cada 45 minutos
    ...
    // Slots devem começar em 08:00, 08:45, 09:30, 10:15, 10:45
