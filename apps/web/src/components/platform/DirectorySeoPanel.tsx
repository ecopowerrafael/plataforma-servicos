...
const failedAggregateJob = z.object({ categoryId: z.string(), citySlug: z.string(), attempts: z.number(), lastError: z.string().nullable(), nextAttemptAt: z.string().nullable() });
...
export function DirectorySeoPanel() {
  ...
  const [maintenancePhase, setMaintenancePhase] = useState<'idle' | 'seo' | 'aggregates' | 'done' | 'incomplete'>('idle');
  ...
  const runAggregatesBatch = () =>
    httpClient.request('/platform/directory/maintenance/aggregates/process-batch', { method: 'POST', schema: aggregatesBatchResult });
  ...
  const retryFailedAggregates = useMutation({
    mutationFn: () => httpClient.request('/platform/directory/maintenance/aggregates/retry-failed', { method: 'POST', schema: retryFailedResult }),
  ...
  <>
    ...
    <p>
      Categorias: {maintenance.data?.sitemap.categoryCount ?? 0} ·
      Cidades: {maintenance.data?.sitemap.cityCount ?? 0} ·
      Negócios: {maintenance.data?.sitemap.businessCount ?? 0}
    </p>
    <p>
      Total de URLs: {maintenance.data?.sitemap.totalUrls ?? 0} ·
      Páginas: {maintenance.data?.sitemap.pageCount ?? 0}
      {' '}(tamanho de página: {maintenance.data?.sitemap.pageSize ?? 0})
    </p>
    <p><small>O sitemap público usa cache de até 6h (s-maxage=21600) — depois de processar, os números acima já refletem o banco, mas /sitemap-directory.xml pode continuar servindo uma resposta em cache por um tempo.</small></p>

    <h4>Fila de agregados por cidade</h4>
    <p>
      Pendentes: {maintenance.data?.cityAggregates.pendingCount ?? 0} ·
      Processando: {maintenance.data?.cityAggregates.processingCount ?? 0}
    </p>
    <p>
      Falhas retentáveis: {maintenance.data?.cityAggregates.failedRetryableCount ?? 0} ·
      Falhas definitivas (5+ tentativas): {maintenance.data?.cityAggregates.failedPermanentCount ?? 0}
    </p>
    {maintenance.data !== undefined && maintenance.data.cityAggregates.failedSample.length > 0 ? (
      <details>
        <summary>Amostra dos jobs com falha (causa)</summary>
        <table>
          <thead><tr><th>Categoria</th><th>Cidade</th><th>Tentativas</th><th>Próxima tentativa</th><th>Erro</th></tr></thead>
          <tbody>
    ...
    {(maintenance.data?.cityAggregates.failedCount ?? 0) > 0 ? (
      <button type="button" onClick={() => retryFailedAggregates.mutate()} disabled={retryFailedAggregates.isPending}>
    ...
    <div className="platform-actions">
      <button type="button" onClick={() => void processOneBatch()} disabled={maintenanceRunning}>Processar SEO</button>
      <button type="button" onClick={() => void processAll()} disabled={maintenanceRunning}>Processar todos</button>
      <button type="button" onClick={pauseMaintenance} disabled={!maintenanceRunning}>Pausar</button>
    </div>
    {maintenanceRunning || maintenanceTotal > 0 ? (
      <>
        <p>{maintenanceProcessed} / {maintenanceTotal} processados{maintenanceErrors > 0 ? ` · ${maintenanceErrors} erros` : ''}</p>
    ...
    {maintenancePhase === 'done' ? (
      <p>
        ...
        <a href="/sitemap-directory.xml" target="_blank" rel="noreferrer">/sitemap-directory.xml</a> ·{' '}
        <a href="/sitemap-directory-1.xml" target="_blank" rel="noreferrer">/sitemap-directory-1.xml</a>
      </p>
    ) : null}
    {maintenancePhase === 'incomplete' ? (
      <p>
        Processamento parado, mas ainda restam pendências ou falhas na fila de agregados — confira os números acima
        antes de considerar concluído. Use &quot;Tentar falhas novamente&quot; se houver jobs com falha.
      </p>
    ) : null}
  </>
) : null}
