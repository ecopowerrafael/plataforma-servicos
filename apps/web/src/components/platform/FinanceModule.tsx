        ...
        <div className="platform-finance-grid">
          {query.data?.configs.map((config) => (
            <article className="platform-panel" key={config.provider}>
              ...
              {config.provider === 'stripe' && config.webhookUrl ? <small>URL do webhook: {config.webhookUrl}{config.lastWebhookEventAt ? ` · último evento ${formatDate(config.lastWebhookEventAt)}` : ''}</small> : null}
