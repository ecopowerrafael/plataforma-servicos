          ...
          {query.data?.configs.map((config) => (
            <article className="platform-panel" key={config.provider}>
              ...
              <dl className="platform-details">
                ...
                {config.provider === 'stripe' ? <><div><dt>Webhook</dt><dd>{config.webhookStatus === 'configured' ? 'Configurado' : 'Não configurado'}</dd></div><div><dt>Customer Portal</dt><dd>{config.customerPortalConfigured ? 'Disponível' : 'Pendente'}</dd></div></> : null}
              </dl>
              {config.provider === 'stripe' && config.webhookUrl ? <small>URL do webhook: {config.webhookUrl}{config.lastWebhookEventAt ? ` · último evento ${formatDate(config.lastWebhookEventAt)}` : ''}<br /><button type="button" onClick={() => { void navigator.clipboard?.writeText(config.webhookUrl!); }}>Copiar URL</button></small> : null}
              <button onClick={() => { configure(config.provider); }} type="button">Configurar</button>
              {config.provider === 'stripe' && config.hasCredentials ? <><button disabled={stripeTest.isPending} onClick={() => { void stripeTest.mutateAsync(); }} type="button">{stripeTest.isPending ? 'Testando…' : 'Testar conexão'}</button><button disabled={stripeSync.isPending} onClick={() => { void stripeSync.mutateAsync(config.environment); }} type="button">{stripeSync.isPending ? 'Sincronizando…' : 'Sincronizar catálogo'}</button><button disabled={stripeWebhook.isPending} onClick={() => { void stripeWebhook.mutateAsync(); }} type="button">{stripeWebhook.isPending ? 'Recriando webhook…' : config.webhookStatus === 'configured' ? 'Reconfigurar webhook' : 'Configurar webhook'}</button></> : null}
            </article>
          ))}
          <article className="platform-panel">
            <header>
