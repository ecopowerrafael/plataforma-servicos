    ...
    <section className="platform-form" aria-label="Integrações">
      ...
      {canManage ? (
        <div className="form-row">
          ...
          <button
            type="button"
            onClick={() => { saveWhatsapp.mutate(!(whatsapp.data?.active ?? false)); }}
          >
      ...
      {canManage ? (
        <div className="form-row">
          ...
          />
          <button type="button" onClick={() => { createExternal.mutate(); }}>
            Adicionar webhook
          </button>
        </div>
      ) : null}
      <ul>
        {external.data?.items.map((item) => (
          <li key={item.publicId}>
            ...
            {canManage ? (
              <button type="button" onClick={() => { remove.mutate(item.publicId); }}>
                Remover
              </button>
            ) : null}
          </li>
