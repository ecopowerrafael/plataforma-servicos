...
export function SubscriptionStatusHero({ plan, subscription, commercial, onPay }: any) {
  ...
  return <section className={`billing-hero billing-hero--${status}`}>
    ...
    <div className="billing-hero__content">
      ...
      <h2>{plan.name}</h2>
      <p className="billing-hero__price"><strong>{money}</strong> <span>/ {subscription.billingCycle === 'ANNUAL' ? 'ano' : subscription.billingCycle === 'MONTHLY' ? 'mês' : subscription.billingCycle.toLowerCase()}</span></p>
