...
function WhatsAppProvidersModule() {
  ...
  const providerError = (error: unknown, fallback: string) => error instanceof HttpError && error.message.trim() !== '' ? error.message : fallback;
  const update = useMutation({ mutationFn: (item: { provider: string; enabled: boolean; baseUrl?: string; apiKey?: string }) => httpClient.request(`/platform/settings/whatsapp/providers/${item.provider}`, { method: 'PUT', body: item }), onSuccess: () => { setMessage('Configuração salva.'); void query.refetch(); }, onError: (error) => { setMessage(`${providerError(error, 'Não foi possível salvar a configuração.')} A alteração foi desfeita.`); void query.refetch(); } });
  const test = useMutation({ mutationFn: (provider: string) => httpClient.request(`/platform/settings/whatsapp/providers/${provider}/test`, { method: 'POST' }), onSuccess: () => { setMessage('Conexão aprovada.'); void query.refetch(); }, onError: (error) => setMessage(providerError(error, 'Não foi possível validar a conexão.')) });
  const busy = update.isPending || test.isPending;
  return <section><PageHeader title="WhatsApp / Provedores" description="Controle a disponibilidade para novas configurações de estabelecimentos; isso não desconecta tenants existentes." />{query.isPending ? <i className="platform-skeleton" /> : query.error instanceof Error ? <ErrorState message={query.error.message} retry={() => void query.refetch()} /> : <article className="platform-panel"><p role="status">{message}</p>{query.data?.items.map((item) => <div key={item.provider} style={{ display: 'grid', gap: 8, padding: '16px 0', borderBottom: '1px solid var(--border-color, #ddd)' }}><strong>{item.provider === 'EVOLUTION' ? 'Evolution GO' : item.provider === 'META' ? 'Meta Oficial' : 'W-API'}</strong><label htmlFor={`provider-enabled-${item.provider}`}><input id={`provider-enabled-${item.provider}`} type="checkbox" checked={item.enabled} disabled={busy} onChange={(event) => update.mutate({ provider: item.provider, enabled: event.target.checked })} /> Disponível para novos estabelecimentos</label>{item.provider === 'EVOLUTION' ? <><label htmlFor="evolution-base-url">URL da API Evolution GO</label><input id="evolution-base-url" aria-describedby="evolution-base-url-help" defaultValue={item.baseUrl ?? ''} placeholder="https://evolution.exemplo" disabled={busy} onBlur={(event) => { if (event.target.value !== (item.baseUrl ?? '')) update.mutate({ provider: item.provider, enabled: item.enabled, baseUrl: event.target.value }); }} /><small id="evolution-base-url-help">Endereço público do servidor Evolution Go.</small><label htmlFor="evolution-api-key">API key administrativa do Evolution GO</label><input id="evolution-api-key" aria-describedby="evolution-api-key-help" type="password" placeholder={item.apiKeyConfigured ? 'Configurada; deixe vazio para preservar' : 'Não configurada'} disabled={busy} onBlur={(event) => { if (event.target.value) update.mutate({ provider: item.provider, enabled: item.enabled, apiKey: event.target.value }); }} /><small id="evolution-api-key-help">Campo write-only; a chave nunca é devolvida nem exibida.</small><span>API key configurada: {item.apiKeyConfigured ? 'sim' : 'não'}</span><span>Estado: {item.configurationStatus === 'READY' ? (item.lastHealthStatus === 'ONLINE' ? 'conexão aprovada' : 'configuração completa; teste pendente') : 'configuração incompleta'}</span><button type="button" onClick={() => test.mutate(item.provider)} disabled={busy || !item.configured}>Testar conexão</button>{!item.configured ? <small>Preencha a URL e a API key antes de testar.</small> : null}<span>Último teste: {item.lastHealthStatus ?? 'não testado'}</span></> : item.provider === 'META' ? <small>Configuração Meta Oficial é gerenciada separadamente; este controle afeta apenas a disponibilidade.</small> : <small>A Master API Key da W-API é configurada no card abaixo; ative somente após configurá-la.</small>}</div>)}</article>}</section>;
}
