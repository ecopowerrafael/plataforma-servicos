...
function WhatsAppProvidersModule() {
  ...
  const update = useMutation({ mutationFn: (item: { provider: string; enabled: boolean; baseUrl?: string; apiKey?: string }) => httpClient.request(`/platform/settings/whatsapp/providers/${item.provider}`, { method: 'PUT', body: item }), onSuccess: () => { setMessage('Configuração salva.'); void query.refetch(); }, onError: () => { setMessage('Não foi possível salvar. A alteração foi desfeita.'); void query.refetch(); } });
  const test = useMutation({ mutationFn: (provider: string) => httpClient.request(`/platform/settings/whatsapp/providers/${provider}/test`, { method: 'POST' }), onSuccess: () => { setMessage('Conexão aprovada.'); void query.refetch(); }, onError: () => setMessage('Não foi possível validar a conexão.') });
