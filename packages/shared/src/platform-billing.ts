...
export const PlatformPaymentProviderSchema=z.enum(['pix-local','mercadopago','stripe']);
...
export const PlatformFinanceConfigProviderSchema=z.enum(['pix-local','mercadopago','stripe']);
export const PlatformFinanceConfigSchema=PlatformPaymentConfigSchema.extend({provider:PlatformFinanceConfigProviderSchema,webhookUrl:z.string().url().nullable().optional(),webhookEndpointId:z.string().nullable().optional(),webhookStatus:z.string().nullable().optional(),lastWebhookEventAt:z.iso.datetime({offset:true}).nullable().optional(),customerPortalConfigured:z.boolean().optional()});
export const PlatformFinanceOverviewSchema=z.object({configs:z.array(PlatformFinanceConfigSchema),manualActivationEnabled:z.boolean()});
